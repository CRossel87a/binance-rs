...
use crate::util::{build_request, build_signed_request_async};
...
impl FuturesMarket {
    ...
    ) -> crate::errors::Result<KlineSummaries>
    where
        S1: Into<String>,
        S2: Into<String>,
        S3: Into<Option<u16>>,
        ...
        S5: Into<Option<u64>>,
    {
        ...
        let data: Vec<Vec<Value>> = self
            .client
            .get(API::Futures(Futures::Klines), Some(request)).await.map_err(|e| crate::errors::Error::from(e.to_string()))?;

        let klines = KlineSummaries::AllKlineSummaries(
            data.iter()
                .map(|row| row.try_into())
                .collect::<crate::errors::Result<Vec<KlineSummary>>>()?,
        );
